...
/datum/controller/subsystem/overmap/Initialize(start_timeofday)
	...
	for(var/shuttle in SSshuttle.mobile_docking_ports)
...
/datum/controller/subsystem/overmap/proc/setup_shuttle_ship(obj/docking_port/mobile/shuttle)
	...
	if(docked_object)
		var/obj/structure/overmap/ship/simulated/S = new (docked_object, shuttle.shuttle_id, shuttle)
		S.docked = docked_object
		shuttle.current_ship = S
	else if(SSmapping.level_trait(shuttle.z, ZTRAIT_RESERVED))
		shuttle.current_ship = new /obj/structure/overmap/ship/simulated(get_unused_overmap_square(), shuttle.shuttle_id, shuttle)
		shuttle.current_ship.state = OVERMAP_SHIP_FLYING
	else if(is_centcom_level(shuttle.z))
		shuttle.current_ship = new /obj/structure/overmap/ship/simulated(null, shuttle.shuttle_id, shuttle)
	else
...
/datum/controller/subsystem/overmap/proc/spawn_events_in_orbits()
	var/orbits = assoc_to_keys(SSovermap.radius_tiles.Copy())
	...
	for(var/i = 1, i <= max_clusters, i++)
		...
		if (LAZYLEN(orbits) == 0)
			...
			orbits = assoc_to_keys(SSovermap.radius_tiles.Copy())
			orbits = orbits - full_orbits
...
/datum/controller/subsystem/overmap/proc/spawn_ruin_levels_in_orbits()
	var/list/orbits = assoc_to_keys(SSovermap.radius_tiles)
...
/datum/controller/subsystem/overmap/proc/spawn_dynamic_encounter(planet_type, ruin = TRUE, dock_id, size = world.maxx / 4, obj/docking_port/mobile/visiting_shuttle, ignore_cooldown = FALSE)
	...
	var/list/ruin_list = SSmapping.themed_ruins[ZTRAIT_SPACE_RUINS]
	...
	if(planet_type)
		switch(planet_type)
			...
			if(DYNAMIC_WORLD_SAND)
				ruin_list = SSmapping.themed_ruins[ZTRAIT_SAND_RUINS]
			...
			// 	ruin_list = SSmapping.themed_ruins[ZTRAIT_JUNGLE_RUINS]
	...
	var/datum/turf_reservation/encounter_reservation = SSmapping.request_turf_block_reservation(total_size, total_size, turf_type_override = /turf/closed/indestructible/blank, area_type_override = target_area)
	if(mapgen)
		var/list/same_area_turfs = list()
		for(var/turf in encounter_reservation.pre_cordon_turfs)
			var/turf/T = turf
			var/area/A = T.loc
	...
	var/turf/bl_offset = encounter_reservation.bottom_left_turfs[0]
	...
	if(ruin_type) //Does AFTER the turfs are reserved so it can find where the allocation is
		...
		var/turf/ruin_turf = locate(
			bl_offset.x + dock_size + 2,
			bl_offset.y + dock_size,
			bl_offset.z
			)
	...
	var/turf/docking_turf = locate(bl_offset.x + dock_size, bl_offset.y + FLOOR(dock_size / 2, 1), bl_offset.z)
	...
	dock.shuttle_id = "[PRIMARY_OVERMAP_DOCK_PREFIX]_[dock_id]"
	...
	var/turf/secondary_docking_turf = locate(
		bl_offset.x + dock_size,
		bl_offset.y + CEILING(dock_size * 1.5, 1),
		bl_offset.z)
	...
	secondary_dock.shuttle_id = "[SECONDARY_OVERMAP_DOCK_PREFIX]_[dock_id]"
...
/datum/controller/subsystem/overmap/proc/get_unused_overmap_square_in_radius(radius, thing_to_not_have = /obj/structure/overmap, tries = MAX_OVERMAP_PLACEMENT_ATTEMPTS)
	...
	if(!radius)
		var/list/K = assoc_to_keys(SSovermap.radius_tiles)
		K -= "unsorted"
		radius = pick(K)
