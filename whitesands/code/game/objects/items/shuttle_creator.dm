...
/datum/armor/shuttle_creator_tool_armor
	fire = 100
	acid = 50
...
/obj/item/shuttle_creator
	...
	armor_type = /datum/armor/shuttle_creator_tool_armor
...
/obj/item/shuttle_creator/afterattack(atom/target, mob/user, proximity_flag)
	...
	if(istype(target, /obj/machinery/door/airlock))
		...
		var/obj/docking_port/port = SSshuttle.get_containing_shuttle(target)
		if(SSshuttle.getShuttle(linkedShuttleId) != port)
			to_chat(user, "<span class='warning'>Error, helm not in linked shuttle!</span>")
			return
		console.set_ship(linkedShuttleId)
		to_chat(user, "<span class='notice'>Console linked successfully!</span>")
		return
